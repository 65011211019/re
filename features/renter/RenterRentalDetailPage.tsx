...
export const RenterRentalDetailPage: React.FC = () => {
    ...
    if (realtimeRental && rental) {
      ...
      if (rental.rental_status !== realtimeRental.rental_status) {
        ...
        if (message) {
          setRealtimeNotification({ 
            isVisible: true, 
            message, 
            type: realtimeRental.rental_status === RentalStatus.LATE_RETURN ? 'warning' : 'success' 
          });
      ...
      if (rental.payment_status !== realtimeRental.payment_status) {
        if (realtimeRental.payment_status === 'paid') {
          setRealtimeNotification({ isVisible: true, message: 'การชำระเงินได้รับการยืนยันแล้ว', type: 'success' });
          setTimeout(() => setRealtimeNotification(prev => ({ ...prev, isVisible: false })), 5000);
        } else if (realtimeRental.payment_status === 'pending_verification') {
          setRealtimeNotification({ isVisible: true, message: 'การชำระเงินอยู่ระหว่างการตรวจสอบ', type: 'info' });
          setTimeout(() => setRealtimeNotification(prev => ({ ...prev, isVisible: false })), 5000);
        }
      }
    }
  ...
  const fetchRentalDetails = () => {
    ...
    getRentalDetails(rentalId, user.id, 'renter')
      ...
      .catch(err => setError((err as ApiError).message || "ไม่สามารถโหลดรายละเอียดการเช่าได้"))
  ...
  useEffect(() => {
    if (token) {
      ...
      return () => { socketService.off('product_updated'); };
    }
  }, [token, rental]);
