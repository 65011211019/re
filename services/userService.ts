...
export const checkWishlistStatus = async (productId: number): Promise<{ isInWishlist: boolean }> => {
  const res = await api.get(`/users/me/wishlist/${productId}/status`);
  return res.data;
};

export const getWishlist = async (page = 1, limit = 10) => {
  try {
    const response = await api.get<ApiResponse<any>>(`/users/me/wishlist?page=${page}&limit=${limit}`);
    return response.data.data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      throw {
...
export const getPublicUserProfile = async (userId: number): Promise<User> => {
  try {
    const response = await api.get<ApiResponse<{ user: User }>>(`/users/${userId}/public-profile`);
    return response.data.data.user;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      throw {
        message: error.response?.data?.message || 'Failed to fetch public user profile',
        status: error.response?.status || 500
      } as ApiError;
    }
    throw error;
