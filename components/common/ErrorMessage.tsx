...
export const ErrorMessage: React.FC<ErrorMessageProps> = ({ message, title, onDismiss }) => {
  const displayTitle = title || 'ข้อผิดพลาด';
  ...
  return (
    <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 my-4 rounded-md" role="alert">
      <div className="flex">
        ...
        {onDismiss && (
          <button onClick={onDismiss} className="ml-auto -mx-1.5 -my-1.5 bg-red-100 text-red-500 rounded-lg focus:ring-2 focus:ring-red-400 p-1.5 hover:bg-red-200 inline-flex h-8 w-8" aria-label="ปิด">
